...
use backend\models\MgfApplicantSearch;
...
class MgfApplicantController extends Controller{
    ...
    public function actionMgf1($id){
        //$userid=Yii::$app->user->identity->id;
        //$applicant=MgfApplicant::find()->where(["user_id"=>$userid])->one();
        if (MgfApplicant::find()->where(['user_id'=>$id])->exists()) {
            $applicant=MgfApplicant::find()->where(['user_id'=>$id])->one();
            
            if ($applicant->district_id>1) {
                //if ($applicant->title=="") {
                    return $this->render('update', ['model' => $applicant,]);
            //include('profile.php');
            } 
            return $this->render('mgf');
        }
