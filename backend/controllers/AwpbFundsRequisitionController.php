...
class AwpbFundsRequisitionController extends Controller {
    ...
    public function actionFrd() {
        ...
        $returnpage="/home/home";
        if (User::userIsAllowedTo('Request Funds')||User::userIsAllowedTo('Manage AWPB') || User::userIsAllowedTo('Manage PW AWPB')) {
...
//                              }
                ...
                return $this->redirect(['awpb-funds-requisition/qofr',
                            // 'searchModel' => $searchModel,
                            // 'dataProvider' => $dataProvider,
                            'id' => $template_model->quarter,
                            'id2' => $template_model->quarter,
    ...
    public function actionQofr_1($id, $id2, $id3) {
        $quarter = 0;
        $template_model = \backend\models\AwpbTemplate::find()->where(['status' => \backend\models\AwpbTemplate::STATUS_CURRENT_BUDGET])->one();
        if (!empty($template_model)) {
            $quarter = $template_model->quarter;
    ...
    public function actionQofri_1($id) {
        $quarter = "";
        return $this->render('qofri_12', [
                    // 'searchModel' => $searchModel,
                    // 'dataProvider' => $dataProvider,
                    'id' => $id,
                    // 'id2' => $id2,
    ...
    public function actionQofrd($id, $id2) {
        ...
        if ((User::userIsAllowedTo('Disburse Funds') || User::userIsAllowedTo('Approve Funds Requisition')) && ($user->province_id == 0 || $user->province_id == '')) {
            if ($cost_centre_id == 1) {
                ...
                $query->select(['awpb_funds_requisition.awpb_template_id', 'awpb_funds_requisition.district_id',
                    'SUM(mo_1_amount) as mo_1_amount',
                    'SUM(mo_2_amount) as mo_2_amount', 'SUM(mo_3_amount) as mo_3_amount', 'SUM(quarter_amount) as quarter_amount']);
                // $query->join('LEFT JOIN', 'awpb_district', 'awpb_district.district_id = awpb_funds_requisition.district_id');
                $query->where(['=', 'awpb_funds_requisition.awpb_template_id', $template_model->id]);
                ...
                $searchModel = new AwpbFundsRequisition();
                $query = $searchModel::find();
                $query->select(['awpb_funds_requisition.awpb_template_id', 'awpb_funds_requisition.cost_centre_id',
                    'SUM(mo_1_amount) as mo_1_amount',
                    'SUM(mo_2_amount) as mo_2_amount', 'SUM(mo_3_amount) as mo_3_amount', 'SUM(quarter_amount) as quarter_amount']);
                //  $query->join('LEFT JOIN', 'awpb_district', 'awpb_district.district_id = awpb_funds_requisition.district_id');
                $query->where(['=', 'awpb_funds_requisition.awpb_template_id', $template_model->id]);
