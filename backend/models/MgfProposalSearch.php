...
class MgfProposalSearch extends MgfProposal{
    ...
    public function searchConceptNotes($params){
        $usertype=Yii::$app->user->identity->type_of_user;
        if ($usertype=="Provincial user") {
            $provinceid=Yii::$app->user->identity->province_id;
            $query = MgfProposal::find()->where(['province_id'=>$provinceid])->where(['proposal_status'=>'Compliant','is_active'=>1]);
        }else if($usertype=="District user") {
            $districtid=Yii::$app->user->identity->district_id;
            //$compliant=MgfApplication::find()->where(['application_status'=>'Compliant'])->all();
            $query = MgfProposal::find()->where(['district_id'=>$districtid])->where(['proposal_status'=>'Submitted','is_concept'=>1,'is_active'=>1])->orWhere(['proposal_status'=>'Under_Review','is_concept'=>1,'is_active'=>1]);
        }else if($usertype=="Other user") {
            $query = MgfProposal::find()->where(['proposal_status'=>'Confirmed','is_active'=>1]);
        }else{
    ...
    public function searchConceptNotesRejected($params){
        $usertype=Yii::$app->user->identity->type_of_user;
        if ($usertype=="Provincial user") {
            $provinceid=Yii::$app->user->identity->province_id;
            $query = MgfProposal::find()->where(['province_id'=>$provinceid])->where(['proposal_status'=>'Sent-Back','is_active'=>1]);
        }else if($usertype=="District user") {
            $districtid=Yii::$app->user->identity->district_id;
            $query = MgfProposal::find()->where(['district_id'=>$districtid])->where(['proposal_status'=>'Sent-Back','is_concept'=>1,'is_active'=>1])->orWhere(['proposal_status'=>'Under_Review','is_concept'=>1,'is_active'=>1]);
        }else if($usertype=="Other user") {
            $query = MgfProposal::find()->where(['proposal_status'=>'Confirmed','is_active'=>1]);
        }else{
            $query = MgfProposal::find()->where(['proposal_status'=>'Sent-Back','is_active'=>1]);
        }
        //$query = MgfProposal::find();
...
<<<<<<< HEAD
            ...
            $districtid=Yii::$app->user->identity->district_id;
            $query = MgfProposal::find()->where(['district_id'=>$districtid])->where(['is_concept'=>0,'is_active'=>1]);
        }else if($usertype=="Other user") {
