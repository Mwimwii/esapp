...
echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
     ...
     if ((\backend\models\User::userIsAllowedTo('Manage AWPB')&& $user->district_id>0 ||$user->district_id!='')|| 
         ...
         (User::userIsAllowedTo('Approve AWPB - PCO') && ( $user->province_id == 0 || $user->province_id == ''))) {
           ...
           if(User::userIsAllowedTo('Approve AWPB - PCO')&&($template_model->incorpation_deadline_pco_moa_mfl) >= strtotime($today) && ( $user->province_id == 0 || $user->province_id == '')) {
...
if (\backend\models\User::userIsAllowedTo('Manage AWPB')&&strtotime($template_model->submission_deadline) >= strtotime($today)&& $user->district_id>0 ||$user->district_id!='') {
