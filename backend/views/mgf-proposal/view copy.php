...
<?php if($model->proposal_status=="Deferred" && $usertype=="Applicant"):?>
<div class="panel with-nav-tabs panel-primary">
<div class="panel-body">
<h3>PROPOSAL SCREENING</h3>
<table class="table table-hover">
<tbody>
    <?php  $i=1;?>
    <?php foreach($categories as $post):?>
        <tr class="table-active">
        <th scope="row"><?=$i.'.';?></th>
        <?php $subtotal=MgfProposalEvaluation::find()->joinWith('criterion')->where(['proposal_id'=>$model->id,'category_id'=>$post->id])->sum('awardedscore');?>
        <td><strong><?=$post->category;?></strong> <label class="class label-info"> <?php if($subtotal!=NULL){ ?> <?=$subtotal.'%';?><?php } ?> </label>
                <table class="table table-hover">
                    <tbody>
                        ...
                        <?php $ppe=MgfProposalEvaluation::find()->joinWith('criterion')->where(['proposal_id'=>$model->id,'category_id'=>$post->id])->andWhere(['NOT',['grade'=>NULL]])->all();?>
